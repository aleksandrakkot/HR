...
void hrms::print_map()
{
    ...
    for(auto m : map_workers)
    {
        string depart = m.first;
        for(auto x : m.second)
        {
            cout << setw(10)<<depart <<setw(10) << x << endl;
        }
    }
}
...
void hrms::changeSalary(string employeeId, double sal)
{
    if(sal < 0)
    {
        throw NegativeValue();
